...
@dataclass
class args:
    result_folder = '../results/age_groups_80PLUS/'
    figPath = os.path.join(result_folder, 'figures_morris')
    checkpoint_folder = os.path.join(result_folder, 'checkpoints')
    input_filePath = '../2022_May_age_groups/Top_100_80PLUS.csv'

    configPath = '../configurations/age_groups_80PLUS.json'
    model_path = os.path.join(checkpoint_folder, os.listdir('../results/age_groups_80PLUS/checkpoints')[0])
...
result_df.to_csv(os.path.join(args.figPath, 'Morris_80PLUS.csv'), index=False)
result_df
