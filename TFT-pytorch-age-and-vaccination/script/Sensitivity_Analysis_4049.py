...
@dataclass
class args:
    result_folder = '../results/age_groups_4049/'
    figPath = os.path.join(result_folder, 'figures_morris')
    checkpoint_folder = os.path.join(result_folder, 'checkpoints')
    input_filePath = '../2022_May_age_groups/Top_100_4049.csv'

    configPath = '../configurations/age_groups_4049.json'
    model_path = os.path.join(checkpoint_folder, os.listdir('../results/age_groups_4049/checkpoints')[0])
...
result_df.to_csv(os.path.join(args.figPath, 'Morris_4049.csv'), index=False)
result_df
