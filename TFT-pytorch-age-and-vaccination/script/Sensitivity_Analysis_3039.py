...
@dataclass
class args:
    result_folder = '../results/age_groups_3039/'
    figPath = os.path.join(result_folder, 'figures_morris')
    checkpoint_folder = os.path.join(result_folder, 'checkpoints')
    input_filePath = '../2022_May_age_groups/Top_100_3039.csv'

    configPath = '../configurations/age_groups_3039.json'
    model_path = os.path.join(checkpoint_folder, os.listdir('../results/age_groups_3039/checkpoints')[0])
...
result_df.to_csv(os.path.join(args.figPath, 'Morris_3039.csv'), index=False)
result_df
